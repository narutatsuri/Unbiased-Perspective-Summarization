...
class ExtractThenEvaluate:
    def __init__(self):
        ...
        # Tokenizer
        # self.tokenizer = tiktoken.encoding_for_model(self.config.prompt_model.split(":")[-1])
    ...
    def score(self, source, generated, default_max_context_len=8192):
        ...
        for prompt in self.config.prompts:
            ...
            def get_response():
                task_prompt = self.config.prompts[prompt].replace("{{article}}", article).replace("{{summary}}", generated)

                # Check context length
                if self.config.prompt_model.split(":")[-1] in openai_context:
                    if len(self.tokenizer.encode(task_prompt)) >= openai_context[self.config.prompt_model.split(":")[-1]]["context"]:
                        raise OverflowError("Input prompt too long.")
                elif len(self.tokenizer.encode(task_prompt)) >= default_max_context_len:
                    raise OverflowError("Input prompt too long.")
            ...
            prompt_to_score[prompt] = get_response()
            
        
        return np.mean(list(prompt_to_score.values()))