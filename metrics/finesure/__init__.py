...
ERROR_TYPES = ['out-of-context error', 
               'entity error', 
               'predicate error', 
...
class FineSure:
    """
    Fine-grained faithfulness evaluator for summaries:
      - faithfulness: fraction of summary sentences with no factual error
    """

    def __init__(self):
        """
        Args:
            config: must include `prompt_model` (for get_backbone)
        """
    ...
    def score(self, source: str, summary: str) -> float:
        """
        Compute the faithfulness score of a summary.

        Args:
            source: the original document/text
            summary: the generated summary, one sentence per line

        Returns:
            A float in [0.0, 1.0] indicating the fraction of sentences
            classified as error-free.
        """
        # split summary into sentences
        sentences = summary.strip().splitlines()

        # build and run the fact-checking prompt
        prompt = get_fact_checking_prompt(input=source, sentences=sentences)
        output = self.backbone.infer(prompt)

        # parse the LLM’s JSON/list output into 0/1 labels
        labels, _ = parsing_llm_fact_checking_output(output)

        # compute % of sentences with no error
        faithfulness = compute_faithfulness_percentage_score(labels)
        return faithfulness
